...
<%!public JSONObject LoadSabongBetsReport(JSONObject mainObj, String userid, String datefrom, String dateto) {
      mainObj = DBtoJson(mainObj, "game_report", "select date_format(datetrn, '%m/%d/%y') as 'date', result, date_format(datetrn, '%r') as 'time',concat(fightnumber, if(length(ws_selection) > 0,concat(' (',ws_selection,')',''),'')) as fightnumber, transactionno, bet_amount, "
            + "  eventid, arena, if(bet_choice='M','Meron',if(bet_choice='W','Wala', 'Draw')) as bet_choice, odd, concat(odd,'%') as odds, bet_actual, promo from " 
            ...
            + " round(if(cancelled,0,if(win,win_amount, if(result='D', 0, -lose_amount) )),2) as bet_actual, promo "
 ...
 <%!public JSONObject LoadCasinoBetsReport(JSONObject mainObj, String userid, String datefrom, String dateto) {
      ...
      */
      mainObj = DBtoJson(mainObj, "game_report", "select provider, date_format(gamedate, '%m/%d/%y') as 'date', date_format(gamedate, '%r') as 'time', gamename, totalbets as bet, totalwin as win, winloss as winlose, promo from tblgamesummary where accountid='"+userid+"' and date_format(gamedate, '%Y-%m-%d') between '" + datefrom + "' and '" + dateto + "' order by gamedate asc");
      return mainObj;
 }%>
